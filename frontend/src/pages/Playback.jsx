...
export default function Playback({ selectedDevice, workflowName, onClose, onEditWorkflow }) {
  ...
  const handleAddStepSubmit = (e) => {
    ...
    const updated = [...workflow.steps, newStep];
    saveUpdatedSteps(updated);
  ...
  const fetchWorkflowDetails = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/workflows/${workflowName}`);
  ...
  const startPlayback = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/playback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handlePlaybackStreamEvent = (data) => {
    if (data.status === 'starting') {
      ...
      setStatus('completed');
      setActiveStep(null);
      setLogs((prev) => [...prev, { type: 'success', text: data.message || 'Playback completed.' }]);
    }
      ...
      <div className="xl:col-span-8 space-y-6">
        <div className="glass-panel rounded-2xl p-6">
          ...
          <div className="space-y-3 mb-6">
              ...
              {workflow?.steps.map((step, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Edit and Delete Buttons (disabled while playing) */}
                    {status !== 'playing' && (
            ...
            {/* Add Step Inline Form (disabled while playing) */}
            {status !== 'playing' && (
              <>
                {showAddStepForm ? (
                  <form onSubmit={handleAddStepSubmit} className="p-4 rounded-xl border border-dashed border-slate-800 bg-slate-950/20 space-y-3.5 mt-3 animate-fade-in">
